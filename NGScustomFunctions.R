...
addNewlines <- function(x, ct = 25){
  n = c(diff(cumsum(nchar(x)) %% ct), 0)
  x = paste0(x, ifelse(n == 0, "", ifelse(n > 0, " ", "\n")), collapse = "")
  return(x)
}
...
ensembleToHugo <- function(seuObj,assay){
  ...
  lol$ensembl_gene_id <- lol$ensembl_gene_id[match(lol$ensembl_gene_id, rownames(lol))]
  rownames(seuObj@assays[["RNA"]]@data) <- lol$external_gene_name
}
